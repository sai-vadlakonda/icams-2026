          ...
          <div className="relative mx-auto max-w-3xl">
            ...
            <ul className="space-y-6">
              {IMPORTANT_DATES.map((item, index) => (
                ...
                >
                  <span
                    className={`absolute left-0 sm:left-1/2 sm:-translate-x-1/2 flex h-9 w-9 items-center justify-center rounded-full text-white ${item.highlight ? 'bg-teal-500' : 'bg-primary-500'
                      }`}
                  >
                    <CalendarDays size={15} />
                  </span>
                  <div
                    className={`w-full rounded-xl border p-5 shadow-soft sm:w-[calc(50%-2rem)] ${item.highlight
